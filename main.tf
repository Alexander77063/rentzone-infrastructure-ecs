...
module "security_group" {
  source       = "git@github.com:Alexander77063/terraform-modules.git//security-groups"
  project_name = local.project_name
  environment  = local.environment
  vpc_id       = module.vpc.vpc_id
  ssh_ip       = var.ssh_ip
...
# launch rds instance
module "rds" {
  source                       = "git@github.com:Alexander77063/terraform-modules.git//rds"
  project_name                 = local.project_name
  environment                  = local.environment
  private_data_subnet_az1_id   = module.vpc.private_data_subnet_az1_id
  private_data_subnet_az2_id   = module.vpc.private_data_subnet_az2_id
...
module "ssl_certificate" {
  source            = "git@github.com:Alexander77063/terraform-modules.git//acm"
...
module "application_load_balancer" {
  source                = "git@github.com:Alexander77063/terraform-modules.git//alb"
  project_name          = local.project_name
  environment           = local.environment
...
# create s3 bucket
module "s3_bucket" {
  source               = "git@github.com:Alexander77063/terraform-modules.git//s3"
  project_name         = local.project_name
...
module "ecs_task_execution_role" {
  source               = "git@github.com:Alexander77063/terraform-modules.git//iam-role"
  project_name         = local.project_name
  environment          = local.environment
...
module "ecs" {
  source                       = "git@github.com:Alexander77063/terraform-modules.git//ecs"
  project_name                 = local.project_name
  environment                  = local.environment
...
# create asg
module "asg" {
  source       = "git@github.com:Alexander77063/terraform-modules.git//asg-ecs"
  project_name = local.project_name
  environment  = local.environment
...
module "route_53" {
  source                             = "git@github.com:Alexander77063/terraform-modules.git//route-53"
